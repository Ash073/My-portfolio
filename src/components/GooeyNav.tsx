...
const GooeyNav = ({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        const particle: HTMLSpanElement = document.createElement("span");
  ...
  interface EffectPositionStyles {
    left: string;
    top: string;
    width: string;
    height: string;
  ...
  const updateEffectPosition = (element: HTMLElement) => {
    ...
    const styles: EffectPositionStyles = {
  ...
  const handleClick = (e: HandleClickEvent, index: number) => {
    ...
    if (filterRef.current) {
      const filterElement = filterRef.current as HTMLElement;
      const particles = filterElement.querySelectorAll(".particle");
      particles.forEach((p) => filterElement.removeChild(p));
    }

    if (textRef.current) {
      (textRef.current as HTMLElement).classList.remove("active");

      void (textRef.current as HTMLElement).offsetWidth;
      (textRef.current as HTMLElement).classList.add("active");
    }
  ...
  useEffect(() => {
    ...
    if (activeLi) {
      ...
      (textRef.current as unknown as HTMLElement)?.classList.add("active");
    }
        ...
        <ul ref={navRef}>
          {items.map((item, index) => (
            ...
            >
              <a
                href={item.href}
                onClick={(e) => handleClick({ currentTarget: e.currentTarget.parentElement as HTMLElement }, index)}
